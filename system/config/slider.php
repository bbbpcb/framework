...
$filename=U("|data/configs/slider.{$name}.json");
include U("|gears/configs/slider.{$name}.php");
if($action=="json"){
  $id=I("id");
  if($id){
    if(array_key_exists($id,$config)){
      ...
      $sliders[]=array("name"=>"添加新图片...","type"=>"button","action"=>"append","iconSkin"=>"iconPlus","font"=>array("color"=>"#28a7e1"));
    }else{
      $sliders=array();
    }
  }else{
			...
			?>
			  <div class="slider-wrapper theme-default">
			    ...
			    <div id="slider" class="nivoSlider">
			      <?php $link=&$sliders[$index];if($link["image"]){ ?>
			        <a href="<?=htmlspecialchars($link["address"])?>" title="<?=htmlspecialchars($link["name"])?>" <?=($link["address"]&&(strtolower(substr($link["address"],0,11))!="javascript:"))?" target=\"_blank\"":""?>><img src="<?=htmlspecialchars($link["image"])?>" <?=$link["summary"]?"title=\"".htmlspecialchars($link["summary"])."\"":""?>></a>
			      <?php } ?>
			    </div>
			  </div>
			  <?php if($link["image"]){ ?><div class="Scripted" style="display:none">$('#slider').nivoSlider({});</div><?php }else{ ?><div class="NoImage"><button type="button" class="btn btn-primary" style="padding-left:22px;padding-right:22px;" onclick="TreeViewModify();">修改图片</button></div><?php } ?>
			  <?php
		}else{
			?>
			  <div class="slider-wrapper theme-default">
			    <div class="ribbon"></div>
			    <div id="slider" class="nivoSlider">
			      <?php $i=0;foreach($sliders as $link){if($link["image"]){$i++; ?>
			        <a href="<?=htmlspecialchars($link["address"])?>" title="<?=htmlspecialchars($link["name"])?>" <?=($link["address"]&&(strtolower(substr($link["address"],0,11))!="javascript:"))?" target=\"_blank\"":""?>><img src="<?=htmlspecialchars($link["image"])?>" <?=$link["summary"]?"title=\"".htmlspecialchars($link["summary"])."\"":""?>></a>
			      <?php }} ?>
			    </div>
			  </div>
			  <?php if($i){ ?><div class="Scripted" style="display:none">$('#slider').nivoSlider({});</div><?php }else{ ?><div class="NoImage"><button type="button" class="btn btn-primary" style="padding-left:22px;padding-right:22px;" onclick="TreeViewAppend();">添加图片</button></div><?php } ?>
			  <?php
		}
	}
	exit(0);
  ...
  $folder[]=array("name"=>$title?$title:"新图片","address"=>$address?$address:"javascript:void(0);","image"=>$image,"summary"=>$summary,"type"=>"link","iconSkin"=>"iconLink");
  ...
  if($spliced[0]["name"]==$title&&$spliced[0]["address"]==$address){
    C($filename,J($sliders));
    echo "*删除图片操作执行成功";
  }else{die("*删除图片操作执行失败");}}else{die("*删除图片操作执行失败");}
  exit(0);
}
?>
<?=$HTMLBOF?>
...
<link rel="stylesheet" type="text/css" href="gears/jquery/nivoslider/nivoslider.css">
<link rel="stylesheet" type="text/css" href="gears/jquery/nivoslider/themes/default/default.css">
...
<script type="text/javascript" src="gears/jquery/nivoslider/nivoslider.js"></script>
<style type="text/css">
html,body{margin:0;padding:0px;display:block;overflow:hidden;width:100%;height:100%;}
.ztree li span.button.iconLink_ico_docu{margin-right:5px; background: url(gears/system/system.image.png) no-repeat scroll 0 0 transparent; vertical-align:top; *vertical-align:middle}
...
#MainView .NoImage{margin-top:-10px;padding:90px 0;text-align:center;background:#eee;border-radius:6px;}
    ...
    callback: {
      ...
      beforeDrop: function(treeId, treeNodes, targetNode, moveType){
        ...
        if(window.DragSourceIndex!=window.DragTargetIndex){
          ...
          }else{parent.toast("移动图片操作执行失败,请重试...");}});
        }
        return false;
      },
  ...
  $.dialog({
      ...
      ok: function () {
          ...
          function(data){if(data.substr(0,1)=="*"){
            ...
            }else{parent.toast("添加图片操作执行失败,请重试...");}});
          return true;
      },
      cancelVal: '关闭',
      cancel: true
...
function TreeViewModify(){
  var tree=$.fn.zTree.getZTreeObj("TreeView");
  var nodes = tree.getSelectedNodes();
  if(!nodes.length){alert("请在左侧区域选择需要修改的图片");return;}
  var node=nodes[0];
  var nodeType=node.type?node.type:false;
  if(nodeType!="link"){alert("请在左侧区域选择需要修改的图片");return;}
  ...
  $.dialog({
      ...
      ok: function () {
          ...
          function(data){if(data.substr(0,1)=="*"){
            ...
            tree.selectNode(parentnode);
            TreeViewOnFocus(parentnode);
          }else{parent.toast("修改图片操作执行失败,请重试...");}});
          return true;
      },
      cancelVal: '关闭',
      cancel: true
...
function TreeViewDelete(){
  var tree=$.fn.zTree.getZTreeObj("TreeView");
  var nodes = tree.getSelectedNodes();
  if(!nodes.length){alert("请在左侧区域选择需要删除的图片");return;}
  var node=nodes[0];
  var nodeType=node.type?node.type:false;
  if(nodeType!="link"){alert("请在左侧区域选择需要删除的图片");return;}
  var nodeindex=tree.getNodeIndex(nodes[0]);
  if(!confirm("确定要删除图片 ["+node.name+"] 吗?"))return;
  var parentnode=nodes[0].getParentNode();
  var folder=parentnode.id;
  ...
  function(data){if(data.substr(0,1)=="*"){
    ...
    tree.selectNode(parentnode);
    TreeViewOnFocus(parentnode);
  }else{parent.toast("删除图片操作执行失败,请重试...");}});
}
function TreeViewOnFocus(node){
  ...
  render.html("");
  $.ajax({
    url: "<?=U(array("action"=>"preview"))?>&folder="+parent+"&index="+(isParent?0:nodeindex+1),
    cache: false,
