...
$data=M("chanyeyuan","`id`>0","`id` DESC",$page,5);
$pn=array_shift($data);
param("page",$pn["page"]);
...
<? if($data){ ?>
  ...
  <tr class="SavedRow<?=$row["id"]?>">
    ...
    <td class="Value" style="height:40px;"><a href="<?=U("~/chanyeyuan/{$row["id"]}/")?>" target="_blank">资源配置</a></td>
    <td class="Value" style="height:40px;"><a class="Delete" href="javascript:void(0);" onclick="if(confirm('确定要删除该条项目数据吗?'))BirdPost(this,{'id':'<?=$row["id"]?>'},'clistdelete',false,'正在删除...');">X</a></td>
  </tr>
  <? } ?>
<? }else{ ?>
  <tr>
